...
A custom step type wraps a primitive (such as `k8s_job`, `k8s_job_wait`, `http_request`, `shell_exec`, or `script_exec`) with fixed defaults and pre-configured parameters. This eliminates repetition when the same image, secrets, volumes, or other parameters are shared across many steps.
...
|-------|------|----------|-------------|
| `base` | string | yes | Must be a primitive type: `k8s_job`, `k8s_job_wait`, `http_request`, `shell_exec`, `script_exec`, `gate`, `load_artifact`, `set_fact`, `assert`, or `llm_invoke` |
